...
const ProfileView = () => {
  ...
  const data = [{ key: "Name", value: state[0].employeeName }, { key: "Contact Number", value: state[0].contactNo }, { key: "CNIC Number", value: state[0].cnicNumber }, { key: "Account Number", value: state[0]?.accountNo }, { key: 'Emergency Contact Number', value: state[0]?.emergencyContactNo }, { key: 'Devgate Mailing ID', value: state[0].email }]
  ...
  const dataset = [{ key: "Team", value: state[0].team }, { key: "Role", value: state[0].role }, { key: 'Type', value: state[0].employeeType }, { key: "Level", value: state[0]?.level }, { key: 'Status', value: state[0]?.employeeStatus }, { key: 'Salary', value: state[0]?.salary }, { key: 'Joining Date', value: moment(state[0]?.employmentStartingDate).format('YYYY-MM-DD') }, { key: 'Ending Date', value: moment(state[0]?.employmentEndingDate).format('YYYY-MM-DD') }]
    ...
    <div className="container-fluid">
      <div className="row " style={{ marginTop: "70px" }}>
        <div className="col-6 p-2" style={{ display: 'flex', flexDirection: 'row' }}>
          ...
          <Typography variant='h1' sx={{ fontSize: '24px', lineHeight: '34px' }}>{state[0]?.employeeName}          <Typography variant="h1" sx={{ fontSize: '16px', lineHeight: '21px' }} >({state[0]?.role})</Typography>
          </Typography>
